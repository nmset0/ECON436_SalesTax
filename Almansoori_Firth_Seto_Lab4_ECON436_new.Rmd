...
FinalData$Lead_STF_Real <- FinalData$Lead_STF_Real* (3.75/4.25)
...
df <- df |> select(-Month)
new_df <- df %>% select("TP","EDUHS","TTU", "FA","MAN_employment","AFS_employment","RT_employment","long_Traffic$Value" )
ldf <- log(new_df)
...
1. Coefficients
```{r}
Beta <- round(coefB(Johansen, r=5), 3) # r = number of cointegrating vectors
Beta
```
...
```{r}
vecm <- VECM(ldf, lag=11, r=5, estim="ML", include = "const") # lag = K-1
summary(vecm)
```
