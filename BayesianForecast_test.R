...
Stan_SalesTax_Forecast_2025 <- forecast(sf1, h = 12)
View(as.data.frame(Stan_SalesTax_Forecast_2025))

forecast_plot1 <- autoplot(Stan_SalesTax_Forecast_2025)
forecast_plot1


# SARIMA object
sf2 <- auto.sarima(ts = FinalData$Lead_STF_Real, seasonal = TRUE, iter = 2000, chains = 4)
# regular SARIMA forecasting model
Sarima_SalesTax_Forecast <- forecast(object = sf2, h = 12)
View(as.data.frame(Sarima_SalesTax_Forecast))
